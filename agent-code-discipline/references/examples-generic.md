...
[MEMORY: Strategies for this error]
- For TamperMonkey userscripts that apply CSS classes to DOM elements populated asynchronously (via $.ajax): apply class-independent styles synchronously before the ajax call; apply element-specific styles INSIDE the ajax success callback, after the element text is set. Race condition symptom: styling correct for the page owner's own profile but missing for other profiles.